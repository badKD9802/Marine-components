...
logger = util.TimeCheckLogger(logging.getLogger(__name__), "== MEETING TOOLS ==")
...
async def get_meeting_room_list(floor=None, min_capacity=None, _auth=None):
    ...
    # 더미 데이터 — OracleSearchClient.meetingroom_db_list() 반환 형식과 동일 (room_name, location만)
    mock_room_list = [
...
async def reserve_meeting_room(meetingroom, title, date, start_time, end_time,
    ...
    # 5. 더미 데이터 — xml_parsing_meeting_insert() 반환 형식 기반
    return {
        "status": "success",
