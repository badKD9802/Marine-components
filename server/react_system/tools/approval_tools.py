...
def _fuzzy_search_forms(query, document_list, top_k=10, threshold=0.3):
    """
    difflib 기반 유사도 검색으로 결재 양식을 찾습니다.
    정확 매칭 → 부분 매칭 → 유사도 순 정렬.
    """
    import pandas as pd

    query_clean = query.replace(" ", "").lower()
    form_names = document_list["FORMNAME"].tolist()

    scored = []
    for idx, name in enumerate(form_names):
        name_clean = name.replace(" ", "").lower() if isinstance(name, str) else ""
        # 정확 매칭
        ...
        if query_clean in name_clean or name_clean in query_clean:
            scored.append((idx, 0.9))
            continue
        # SequenceMatcher 유사도
        ratio = SequenceMatcher(None, query_clean, name_clean).ratio()
        if ratio >= threshold:
            scored.append((idx, ratio))

    # 폴더명도 검색
    fldr_names = document_list["FLDRNAME"].tolist()
    matched_indices = {s[0] for s in scored}
    for idx, fldr in enumerate(fldr_names):
        if idx in matched_indices:
            continue
        fldr_clean = fldr.replace(" ", "").lower() if isinstance(fldr, str) else ""
        if query_clean in fldr_clean:
            scored.append((idx, 0.85))
    ...
    return document_list.iloc[top_indices]
...
async def get_approval_form(form_name, department=None, _auth=None):
    ...
    if _auth and _auth.is_authenticated:
        try:
            ...
            # 양식명 검색 (유사도 기반)
            df = _fuzzy_search_forms(form_name, document_list)
            ...
            return {"status": "error", "message": "결재 양식 조회 중 오류가 발생했습니다."}
