...
try:
    ...
    logger.info("[INFO] calendar_html.py 로드 성공")
except ImportError as e:
    CALENDAR_AVAILABLE = False
    logger.warning(f"[WARNING] calendar_html.py를 찾을 수 없습니다: {e}")
...
def _extract_meeting_reservations(meeting_data, meetingroom_name=None):
    ...
    if room_to_fetch:
        logger.info(f"[INFO] meeting_data 불완전 → '{room_to_fetch}' 자동 재조회")
        try:
            from react_system.tools.meeting_tools import get_meeting_rooms
            ...
            logger.warning(f"[WARNING] 자동 재조회 실패: {e}")
...
def format_schedule_as_calendar(schedules, user_name="사용자", start_dt=None, end_dt=None, _auth=None):
    ...
    Returns:
        str: 파일 저장 안내 메시지
    """
    logger.debug("===== format_schedule_as_calendar 함수 진입 =====")
    logger.debug(f"받은 파라미터: schedules={type(schedules)}, user_name={user_name}, start_dt={start_dt}, end_dt={end_dt}")
    try:
        ...
        if isinstance(schedules, dict):
            logger.debug(f"schedules는 dict입니다. keys: {list(schedules.keys())}")
            # query 정보 추출 (있으면)
            if 'query' in schedules:
                query_info = schedules['query']
                logger.debug(f"query 정보 발견: {query_info}")
            # schedules 배열 추출
            if 'schedules' in schedules:
                schedules = schedules['schedules']
                logger.debug(f"schedules 배열 추출, 개수: {len(schedules)}")
        ...
        if query_info:
            try:
                if 'start_dt' in query_info and query_info['start_dt']:
                    ...
                    logger.debug(f"query에서 start_dt 추출 성공: {start_dt}")

                if 'end_dt' in query_info and query_info['end_dt']:
                    end_dt = query_info['end_dt'][:10].replace('-', '.')
                    logger.debug(f"query에서 end_dt 추출 성공: {end_dt}")
            except Exception as e:
                logger.warning(f"query에서 날짜 추출 실패: {e}")
        ...
        logger.debug(f"render_graphic_calendar 호출: title={title}, start_dt={start_dt}, end_dt={end_dt}, schedules 개수={len(schedules)}")
...
def format_schedule_as_table(schedules, user_name="사용자", start_dt=None, end_dt=None, _auth=None):
    ...
    Returns:
        dict: HTML 결과
    """
    logger.debug("===== format_schedule_as_table 함수 진입 =====")
    try:
        # AI가 get_schedule() 결과 전체를 전달한 경우 처리
        query_info = None
        if isinstance(schedules, dict):
...
def format_meeting_rooms_as_calendar(meeting_data, user_name="사용자", meetingroom_name=None, start_dt=None, end_dt=None, _auth=None):
    ...
    Returns:
        str: 파일 저장 안내 메시지
    """
    logger.debug("===== format_meeting_rooms_as_calendar 함수 진입 =====")
    try:
        if not CALENDAR_AVAILABLE:
...
def format_meeting_rooms_as_table(meeting_data, user_name="사용자", meetingroom_name=None, start_dt=None, end_dt=None, _auth=None):
    ...
    Returns:
        dict: HTML 결과
    """
    logger.debug("===== format_meeting_rooms_as_table 함수 진입 =====")
    try:
        WEEKDAYS_KR = ['월', '화', '수', '목', '금', '토', '일']
        _PALETTE = [
