...
async def get_schedule(date=None, date_range_start=None, date_range_end=None,
    ...
    if _auth and _auth.is_authenticated:
        try:
            ...
            return {"status": "error", "message": "일정 조회 중 오류가 발생했습니다. 다시 시도해주세요."}
...
async def create_schedule(title, date, start_time, end_time,
    ...
    if _auth and _auth.is_authenticated:
        try:
            ...
            return {"status": "error", "message": "일정 등록 중 오류가 발생했습니다. 다시 시도해주세요."}
...
async def update_schedule(cal_id=None, title=None, title_chg=None,
    ...
    if _auth and _auth.is_authenticated:
        try:
            ...
            if event_id:
                from app.tasks.node_agent.aiassistant.services.xml_parsing import xml_parsing_result_message
                ...
                response = api.update(**api_params)

                if response == "500":
                    return {"status": "error", "message": "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요."}

                result_msg = await xml_parsing_result_message(response)
                if result_msg == "성공하였습니다.":
                    return {"status": "success", "message": "일정이 수정되었습니다.", "updated_params": update_params}
                elif result_msg == "중복되었습니다.":
                    return {"status": "duplicate", "message": "변경하려는 시간에 이미 다른 일정이 있습니다."}
                elif "권한" in (result_msg or ""):
                    return {"status": "error", "message": "이 일정을 수정할 권한이 없습니다."}
                elif "유효하지 않은" in (result_msg or ""):
                    return {"status": "error", "message": f"유효하지 않은 요청입니다: {result_msg}"}
                else:
                    return {"status": "error", "message": f"일정 수정에 실패했습니다: {result_msg}"}
        except Exception as e:
            logger.error(f"update_schedule API 오류: {e}")
            return {"status": "error", "message": "일정 수정 중 오류가 발생했습니다. 다시 시도해주세요."}
    ...
    if _auth and _auth.is_authenticated:
        try:
            ...
            if event_id:
                ...
                response = api.delete(USERID=_auth.user_id, EVENT_ID=event_id, SELECT_DT=select_dt, K=_auth.k)
                ...
                result_msg = await xml_parsing_result_message(response)
                if result_msg == "성공하였습니다.":
                    return {"status": "success", "message": "일정이 삭제되었습니다.", "deleted": search_params}
                elif "권한" in (result_msg or ""):
                    return {"status": "error", "message": "이 일정을 삭제할 권한이 없습니다."}
                else:
                    return {"status": "error", "message": f"일정 삭제에 실패했습니다: {result_msg}"}
        except Exception as e:
            logger.error(f"delete_schedule API 오류: {e}")
            return {"status": "error", "message": "일정 삭제 중 오류가 발생했습니다. 다시 시도해주세요."}
