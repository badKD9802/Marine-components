...
- 단순 표 표시만 원하면 format_data_as_table 사용""",
    ...
    # ============================================================
    {
        ...
        "function": {
            ...
            "description": """안전관련 법령 및 규정을 검색하고 AI 답변을 생성합니다.

**사용 시나리오:**
- 산업안전보건법, 중대재해처벌법 등 안전 법령 질문
- 안전관리자 선임, 안전교육, 위험성평가 등 안전 규정 질문
- 특정 법조문 조회 (예: "산안법 제17조")
- 안전 관련 의무, 벌칙, 절차 질문

**지원 법령 (18개):**
산업안전보건법, 중대재해처벌법, 재난안전법, 건설기술진흥법,
시설물안전법, 근로기준법, 소방법, 전기안전법, 승강기법 등

**약어 지원:** 산안법, 중처법, 재안법 등 약어를 자동으로 정식 명칭으로 변환합니다.

**주의:** 이 도구는 HTML 형식으로 답변을 생성하여 화면에 직접 표시합니다.
도구 실행 후 html_content가 반환되면, 사용자에게 텍스트로 다시 나열하지 마세요.
간단한 요약이나 추가 안내만 제공하세요.""",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "검색할 질문 (예: '안전관리자 선임 기준', '산안법 제17조')",
                    },
                    "category": {
                        "type": "string",
                        "enum": ["전체", "법령", "정부가이드", "내부규정", "매뉴얼", "현장서식"],
                        "description": "검색 범위. 기본값: '전체'",
                    },
                    "detail_level": {
                        "type": "string",
                        "enum": ["brief", "standard", "detailed"],
                        "description": "답변 상세도. brief=3문장, standard=구조화(기본), detailed=상세",
                    },
                },
                "required": ["query"],
            },
