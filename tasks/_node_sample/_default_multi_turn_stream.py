...
from app.tasks.node_sample.node_generate_stream import node_generate
...
# token을 적당한 크기로 자른다.
# ------------------------------------------------------------------------
def truncate_token_limit__(tokenizer_name, chunk_infos: list[ChunkInfo], token_cnt=1600, merge_cnt=3):
...
# ChatGPT로 응답을 생성한다.
# ------------------------------------------------------------------------
async def node_generate__(stat: LangGraphState, writer: StreamWriter):
    prompt_template = """
    ...
    if question_type == "llm_question":
        ...
        chunk_str = truncate_token_limit__(tokenizer_name, chunks, 2048, 3)
...
async def node_response_fail(stat: LangGraphState, writer: StreamWriter):
    just = JustMessage(stat)
    just.update_answer("질문 메타정보가 부족하여 응답생성을 시작할 수 없습니다.", -1)
