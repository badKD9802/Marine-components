...
logger = util.TimeCheckLogger(logging.getLogger(__name__), "== SERVICE UNLOAD ==")


async def service_unload(stat: LangGraphState):
    # --------- 변수 가져오기 ---------
    ...
    logger.info("========== SERVICE UNLOADING ===============")
    try:
        # -------------------------------------------------------
        # DB에서 Loading으로 Service 상태 바꾸기
        # -------------------------------------------------------
        await worker.delete_worker()
        await service.update_agent_data(session_status, service_name, data_status="unloading", service_status="stopped")
        await service.delete_retriever(service_name)  # 이미 없다. 에러 날 수도 있음.

        # await admin_orm.update_service_status(session_status, service_name, "Stop", True)
        logger.info("========== SERVICE UNLOAD FINISH ===============")
        await service.update_agent_data(session_status, service_name, data_status="unloaded")

    except Exception as e:
        logger.info(f"SERVICE UNLOAD FAIL({service_name})  : {e}")
        await service.update_agent_data(session_status, service_name, data_status="loaded", service_status="stopped")
    ...
    just.append_answer(f"서비스({service_name}) 언로드 성공")
    return_stat = LangGraphState(rag_code="succ")
    logger.info(f"SERVICE UNLOAD SUCCESS ({service_name})")
    return return_stat
...
def build_workflow(workflow, websocket: WebSocket = None):
    ...
    workflow.add_node("SERVICE_UNLOAD", service_unload)

    workflow.add_edge(START, "SERVICE_UNLOAD")
    workflow.add_edge("SERVICE_UNLOAD", END)
