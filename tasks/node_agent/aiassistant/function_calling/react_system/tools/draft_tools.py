...
def _guide_select_type() -> dict:
    ...
    각 유형별 requirements 안내를 미리 숨긴 상태로 포함하여,
    버튼 클릭 시 인라인 JavaScript로 해당 안내를 보여준다.
    """
    ...
    # --- 버튼 onclick JS (숫자 인덱스 기반, 따옴표 충돌 회피) ---
    # 전체 문서 순서: admin + planning (패널도 같은 순서로 생성)
    all_docs = admin_docs + planning_docs

    # 유형 선택 JS: 리스트 숨기고 인덱스로 패널 표시
    # NOTE: 이 문자열은 f-string의 {변수} 치환으로 삽입되므로 JS의 {}가 그대로 출력됨
    show_req_tpl = (
    ...
    # --- 각 유형별 requirements 패널 (숨김 상태, 버튼과 같은 순서) ---
    req_panels = ""
...
def _get_llm_config(_auth=None) -> dict:
    """auth에서 LLM 설정(base_url, api_key, model)을 추출한다.
    JustLLM과 동일한 config.ini 소스를 사용."""
    if _auth and _auth.stat:
        try:
            ...
            config = (
                llm_config.get(default_name, llm_config) if default_name else llm_config
            )
            return {
            ...
            logger.warning(f"LLM config 로드 실패 (env fallback): {e}")
...
플레이스홀더([내용을 작성하세요] 등)를 사용하지 말고, 구체적인 내용으로 채우세요."""
        ...
        # 기획보고서(planning_report)는 기호 체계 강조 + max_tokens 증가
...
def _generate_hwpx_file(
    ...
    서버 엔드포인트/프록시 의존성을 제거하기 위해 파일 내용을
    base64로 인코딩하여 data URI로 직접 전달한다.

    Returns:
        {"data_uri": "data:...", "filename": "...", "title": "..."} 또는 None
    """
    import base64
    import tempfile
...
def _style_doc_symbols(content_escaped: str) -> str:
    ...
    이미 html.escape()된 content를 줄별로 파싱하여 기호별 스타일을 적용한다.

    지원 패턴:
    - 기획보고서: □(대항목) → ○(중항목) → ―(소항목) → ※(참고)
    - 행정문서: 1.(대항목) → 가.(중항목) → 1)(소항목) → 가)(세항목)
    - 공통: Ⅰ.~Ⅹ. 로마숫자 대제목, 아라비아숫자 소제목
    """
    import re as _re
...
def _build_public_doc_html(
    ...
    # 헤더 메타 정보
    meta_parts = [f"📄 {html_mod.escape(doc_type_label)}"]
    ...
    if has_footer:
        ...
        _esc_pos = (
            html_mod.escape(sender.get("position", ""))
            if sender.get("position") and sender.get("position") != "-"
            else ""
        )
  ...
  <div style="margin-top:24px;border-top:1px solid #cbd5e1;padding-top:16px;display:flex;justify-content:flex-end;">
    <table style="width:360px;border-collapse:collapse;font-size:0.86em;color:#1e293b;">
      <tr>
        <td style="{_lbl_style}"></td>
        <td style="{_hdr_style}">담당</td>
        <td style="{_hdr_style}">검토</td>
        <td style="{_hdr_style}">결재</td>
      </tr>
      <tr>
        <td style="{_lbl_style}">직급</td>
        <td style="{_cell_style}">{_esc_pos}</td>
        <td style="{_cell_style}"></td>
        <td style="{_cell_style}"></td>
      </tr>
      <tr>
        <td style="{_lbl_style}">서명</td>
        <td style="{_sign_style}">{_esc_name}</td>
        <td style="{_sign_style}"></td>
        <td style="{_sign_style}"></td>
      </tr>
      <tr>
        <td style="{_lbl_style}">날짜</td>
        <td style="{_cell_style}">{html_mod.escape(today)}</td>
        <td style="{_cell_style}">&nbsp;&nbsp;/&nbsp;&nbsp;/&nbsp;&nbsp;</td>
        <td style="{_cell_style}">&nbsp;&nbsp;/&nbsp;&nbsp;/&nbsp;&nbsp;</td>
      </tr>
    </table>
    ...
    # HWPX 다운로드 버튼 (base64 data URI — 서버 엔드포인트/프록시 불필요)
    hwpx_btn = ""
    if hwpx_url:
...
async def _get_sender_info(_auth=None):
    ...
    try:
        from app.tasks.node_agent.aiassistant.db_extract.db_search_api import (
            OracleSearchClient,
        )
...
def _build_signature(sender: dict, tone: str = "formal") -> str:
    ...
    org = (
        " ".join(filter(None, [dept, team]))
        if (dept and dept != "-") or (team and team != "-")
        else ""
    )
    if org:
        parts.append(org)
...
async def draft_email(
    recipient, purpose, subject=None, key_points=None, tone="formal", _auth=None
):
    """
    업무 메일 초안을 작성합니다.
    ...
    sender_org = (
        " ".join(filter(None, [sender.get("dept", ""), sender.get("team", "")]))
        if sender
        else ""
    )
...
async def draft_document(
    ...
    """
    공식 문서 초안을 작성합니다.
    공공기관 문서 유형은 LLM을 호출하여 행정안전부 공문서 작성 규정에 맞는 실제 내용을 생성합니다.

    Args:
        document_type: 문서 유형 (공문, 협조전, 업무보고서, 기획안, 회의록, 결과보고서, 사업계획서, 검토보고서, 보고서, 기획서, 제안서)
        title: 문서 제목
    ...
    Returns:
        dict: 문서 초안 (공공기관 문서는 html_content 포함)
    """
    ...
    if found:
        ...
        # LLM 호출로 실제 내용 생성
        generated = await _generate_document_content(
            template,
            template["label"],
            title,
        ...
        if generated:
            ...
            html_content = _build_public_doc_html(
                template["label"],
                title,
                generated,
                sender,
                has_footer=template.get("has_footer", True),
                recipient=recipient,
                reference=reference,
                hwpx_url=hwpx_url,
                is_planning_report=is_planning,
            )

            result = {
                "status": "success",
                "html_content": html_content,
                "text_summary": f"[{template['label']}] {title}\n\n{generated}",
                "draft": {
                ...
                "message": f"{template['label']} 초안이 작성되었습니다.",
            }
            if hwpx_url:
                result["hwpx_filename"] = (
                    hwpx_url.get("filename", "") if isinstance(hwpx_url, dict) else ""
                )
            ...
            return {
                ...
                "draft": {
                    ...
                    "body": fallback_body,
                    "generated_by": "Template (LLM 호출 실패)",
                },
                "message": f"{template['label']} 템플릿이 생성되었습니다. (LLM 호출 실패로 구조만 제공)",
            }
    ...
    if not sections:
        sections = _GENERAL_DOC_TEMPLATES.get(
            document_type, ["1. 서론", "2. 본론", "3. 결론"]
        )
...
async def review_document(
    document_content, document_type=None, review_focus="전체", _auth=None
):
    """작성된 문서를 검수하여 개선 사항을 제안합니다.

    형식, 문체, 내용, 어문규범 4개 영역을 평가하고 수정 제안을 제공합니다.

    Args:
        document_content: 검수할 문서 내용 (전체 텍스트)
        document_type: 문서 유형 (예: 정책제안보고서, 공문, 업무보고서 등)
        review_focus: 검수 초점 (전체, 형식, 내용, 기호체계). 기본값: 전체
        _auth: AuthContext (자동 주입)

    Returns:
        dict: 검수 결과 (status, html_content, text_summary, review)
    """
    if not document_content or not document_content.strip():
        return {
            "status": "error",
            "message": "검수할 문서 내용이 비어있습니다.",
        }
    ...
    try:
        ...
        focus_instruction = ""
        if review_focus == "형식":
            focus_instruction = "형식(format) 영역에 집중하여 검수하세요. 다른 영역은 간단히 평가하세요."
        elif review_focus == "내용":
            focus_instruction = "내용(content) 영역에 집중하여 검수하세요. 다른 영역은 간단히 평가하세요."
        elif review_focus == "기호체계":
            focus_instruction = "기호 체계(symbol hierarchy) 영역에 집중하여 검수하세요. 다른 영역은 간단히 평가하세요."
...
{{
  ...
  "issues": [
    {{"area": "<format|style|content|grammar>", "severity": "<high|medium|low>", "description": "<문제 설명>", "suggestion": "<수정 제안>", "location": "<위치 또는 해당 문구>"}}
  ],
  "summary": "<전체 검수 요약 1~2문장>"
}}
```

[규칙]
- issues 배열에 최소 3개, 최대 15개의 이슈를 포함하세요
- severity: high(필수 수정), medium(권장 수정), low(선택적 개선)
- 칭찬할 점도 issues에 severity="low", description에 "잘된 점:" 접두어로 포함 가능
...
{document_content}"""
        ...
        try:
            ...
            logger.warning("검수 결과 JSON 파싱 실패, 원본 응답 반환")
            return {
                "status": "success",
                "html_content": _build_review_html_fallback(
                    raw_response, document_type
                ),
                "text_summary": "문서 검수 완료 (원본 응답 반환)",
                "message": "문서 검수가 완료되었습니다.",
            }
        ...
        total_score = sum(
            scores.get(k, 0) for k in ("format", "style", "content", "grammar")
        )
        max_score = 40

        # HTML 렌더링
        html_content = _build_review_html(
            scores, issues, summary_text, total_score, max_score, document_type
        )

        return {
            "status": "success",
            "html_content": html_content,
            "text_summary": f"검수 완료: 총점 {total_score}/{max_score}점 ({summary_text})",
            "review": {
                "scores": scores,
        ...
        return {
            "status": "error",
            "message": f"문서 검수 중 오류가 발생했습니다: {str(e)}",
        }
...
def _build_review_html(
    ...
    """검수 결과를 HTML로 렌더링한다 (점수 게이지 바 + 이슈 목록)."""
    uid = f"review-{id(summary) % 100000}"
    ...
    def _total_color(score, max_val=40):
        ratio = score / max_val if max_val else 0
        if ratio >= 0.8:
            return "#22c55e"
        elif ratio >= 0.6:
    ...
    for key in ("format", "style", "content", "grammar"):
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
        <span style="width:60px;font-size:0.82em;color:#475569;font-weight:600;">{label}</span>
        <div style="flex:1;height:14px;background:#e2e8f0;border-radius:7px;overflow:hidden;">
          <div style="width:{pct}%;height:100%;background:{color};border-radius:7px;transition:width 0.5s;"></div>
        </div>
        <span style="width:40px;text-align:right;font-size:0.85em;font-weight:700;color:{color};">{score}/10</span>
      </div>"""

    total_color = _total_color(total_score, max_score)
    ...
    for issue in issues:
        ...
        location_html = (
            f'<div style="font-size:0.78em;color:#94a3b8;margin-top:2px;">위치: {location}</div>'
            if location
            else ""
        )
        suggestion_html = (
            f'<div style="font-size:0.82em;color:#1d4ed8;margin-top:3px;">💡 {suggestion}</div>'
            if suggestion
            else ""
        )

        issues_html += f"""
      <div style="padding:8px 12px;border-left:3px solid {'#ef4444' if sev == 'high' else '#f59e0b' if sev == 'medium' else '#22c55e'};background:#f8fafc;border-radius:0 6px 6px 0;margin-bottom:6px;">
        <div style="display:flex;align-items:center;gap:6px;">
          <span>{icon}</span>
          <span style="font-size:0.78em;padding:1px 6px;background:{'#fef2f2' if sev == 'high' else '#fffbeb' if sev == 'medium' else '#f0fdf4'};border-radius:4px;color:{'#dc2626' if sev == 'high' else '#d97706' if sev == 'medium' else '#16a34a'};font-weight:600;">{sev_label}</span>
          <span style="font-size:0.78em;color:#64748b;">[{area}]</span>
        </div>
        <div style="font-size:0.88em;color:#1e293b;margin-top:4px;">{desc}</div>
        {location_html}
        {suggestion_html}
      </div>"""
  ...
  <div style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;background:#fff;">
    <div style="background:linear-gradient(135deg,#1e293b 0%,#334155 100%);color:#fff;padding:10px 18px;">
      <span style="font-size:0.9em;font-weight:600;">📋 문서 검수 결과{doc_type_label}</span>
      <span style="font-size:0.85em;float:right;color:{total_color};font-weight:700;">총점: {total_score}/{max_score}</span>
    </div>
    <div style="padding:16px 20px;">
      <div style="margin-bottom:14px;">
        <div style="font-size:0.85em;color:#64748b;margin-bottom:8px;font-weight:600;">영역별 점수</div>
        {gauge_bars}
      </div>
      <div style="padding:10px 14px;background:#f1f5f9;border-radius:8px;margin-bottom:14px;font-size:0.88em;color:#334155;">
...
def _build_review_html_fallback(
    raw_response: str, document_type: str | None = None
) -> str:
...
if __name__ == "__main__":
    ...
    async def _test():
        ...
        # 테스트 2: 공공기관 공문 초안 (LLM 호출)
        print("\n[테스트 2: 공공기관 공문 초안]")
        result2 = await draft_document(
            document_type="공문",
            title="AI 시스템 도입 관련 협조 요청",
            recipient="각 부서장",
            content_requirements="AI 챗봇 시스템 도입 배경, 협조 요청 사항",
        )
        print(f"상태: {result2['status']}")
        print(f"메시지: {result2['message']}")
        print(f"생성 방식: {result2['draft']['generated_by']}")
        print(f"본문 미리보기:\n{result2['draft']['body'][:300]}...\n")

        # 테스트 3: 공공기관 업무보고서
        print("\n[테스트 3: 업무보고서]")
        result3 = await draft_document(
            document_type="업무보고서", title="2026년 1분기 AI 시스템 구축 현황"
        )
        print(f"상태: {result3['status']}")
        print(f"메시지: {result3['message']}")
        print(f"생성 방식: {result3['draft']['generated_by']}")

        # 테스트 4: 일반 보고서 (하위호환)
        print("\n[테스트 4: 일반 보고서 (하위호환)]")
        result4 = await draft_document(
            document_type="보고서",
            title="2026년 1분기 AI 시스템 구축 추진 현황",
            content_requirements="LangGraph 적용, ReAct 패턴 구현",
        )
        print(f"상태: {result4['status']}")
        print(f"메시지: {result4['message']}")
        print(f"생성 방식: {result4['draft']['generated_by']}")
