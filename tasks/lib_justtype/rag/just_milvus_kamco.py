...
from app.tasks.lib_justtype.common.global_clients import get_openai_client, get_milvus_clients, get_kiwi, get_reranker_client
...
# Kiwi 싱글턴 사용 (global_clients.py에서 관리)
kiwi, stopwords = get_kiwi()
...
class FilterBuilder:
    ...
    def build_filter_expr(filters: Dict) -> str:
        ...
        # 지식베이스 게시판 필터 (board_filter_expr가 있으면 다른 조건 무시)
        if "board_filter_expr" in filters:
            return filters["board_filter_expr"]
...
class JustMilvusKamco:
    ...
    def __init__(self, stat):
        ...
        try:
            ...
            if self.embedding_client:
                ...
                params = {
                    "base_url": self.embedding_config.get("embedding_base_url"),
                    "api_key" : self.embedding_config.get("embedding_api_key")
                }
                self.embedding_client = OpenAI(**params)
        ...
        # 리랭커 설정
        self._reranker_client: Optional[httpx.AsyncClient] = None

        # self.reranker_url = self.rerank_config.get("reranker_base_url", "http://vllm-telepix-pixie-spell-reranker-preview-service.kamco.svc.cluster.local:8000/score")
        # self.reranker_model = self.rerank_config.get("reranker_model", "telepix-pixie-spell-reranker")
        # 본사서버 적용
    ...
    def _init_milvus_client(self) -> Optional[AsyncMilvusClient]:
        """Milvus 클라이언트 초기화 (global_clients 싱글턴 사용)"""
        try:
            client, sync_client = get_milvus_clients(self.milvus_config)
            if client is None:
                raise ConnectionError("Failed to get Milvus clients from singleton")
    ...
    def _init_embedding_client(self) -> Optional[OpenAI]:
        ...
        try:
            ...
            if client:
                ...
                self.embedding_client = get_openai_client(self.embedding_config)
                if self.embedding_client:
                    raise ConnectionError("Failed to get OpenAI client")
        except Exception as e:
            logger.error(f"Failed to initialize embedding client: {e}", exc_info=True)
    ...
    async def _get_reranker_client(self) -> httpx.AsyncClient:
        """리랭커 HTTP 클라이언트 (global_clients 싱글턴 사용)"""
    ...
    def encode(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            # logger.info(f"0 {len(texts_to_embed)} texts in {elapsed:.2f}s")
    ...
    async def encode_with_cache(self, texts: List[str]) -> List[List[float]]:
        """
        임베딩 벡터 변환 (Redis 캐시 사용).
        캐시 히트 시 Redis에서 반환, 미스 시 encode() 호출 후 캐시 저장.
        """
        import hashlib
        import json as _json

        try:
            from app.justtype.redis_client import redis_client
            if redis_client.client is None:
                await redis_client.connect()
        except Exception:
            # Redis 미사용 환경 — 캐시 없이 직접 임베딩
        ...
        for i, text in enumerate(texts):
            text_normalized = str(text).strip() if text and str(text).strip() else " "
            cache_key = f"emb:{model_name}:{hashlib.md5(text_normalized.encode()).hexdigest()}"
            try:
        ...
        if miss_indices:
            ...
            embeddings = await asyncio.to_thread(self.encode, miss_texts)

            for j, idx in enumerate(miss_indices):
                results[idx] = embeddings[j]
                # 비동기 캐시 저장 (fire-and-forget)
                text_normalized = str(texts[idx]).strip() if texts[idx] and str(texts[idx]).strip() else " "
                cache_key = f"emb:{model_name}:{hashlib.md5(text_normalized.encode()).hexdigest()}"
                try:
                    await redis_client.client.set(
                        cache_key,
                        _json.dumps(embeddings[j]),
                        ex=cache_ttl,
                    )
                except Exception:
                    pass  # 캐시 저장 실패 무시
    ...
    async def _perform_hybrid_search(
        ...
        if 'approval' in dense_collection_name or 'approval' in sparse_collection_name:
            ...
            exclude_formid = list(set(['HGW6X120754745904000', 'JHOMS223182178442000', 'HGW6X092540644994000', 'JHOMS201951174542000', 'HGW6X113214388187000', 
                                       'HGW6X171154397472000', 'HGW6X090080053631000', 'JHOMS202942392105000', 'HGW6X140881144319000', 'JHOMS202391723581000', 
                                       'HGW6X163272838104000', 'JHOMS202391723523000', 'HGW6X150270726015000', 'HGW6X163633253626000', 'HGW6X152733299750000', 
                                       'JHOMS201951174458000', 'JHOMS202331651039000', 'HGW6X092540645007000', 'HGW6X092540644983000', 'HGW6X103052054843000']))
                        
            title_conditions = [f'title like "%{kw}%"' for kw in exclude_title]
            formid_conditions = [f'formid == "{kw}"' for kw in exclude_formid]
            ...
            if "doc_range" in filters:
                ...
                add_expr = f" AND level1_tag IN {kms_items} AND status LIKE '%3000%' AND NOT file_name LIKE '%현행 대 개정 대비표%'" # 상태값 3000인 경우만 추출
                expr += add_expr
        ...
        for attempt in range(self.hybrid_config["retry_attempts"] + 1):
            try:
                ...
                if 'approval' in collection_name:
                    search_params = {"metric_type": "COSINE", "params": {"ef": 2048}}
                else:
    ...
    async def get_query(self, collection_name: str, filter_expr:str):
        try:
            result = await self.client.query(
                            collection_name=collection_name,
                            output_fields=["chunk_id", "orig_text", "embed_text"],
                            filter=filter_expr,
                            timeout=self.hybrid_config["timeout"],
                        )
    ...
    async def get_lastest_conds(self, collection_name: str, filter_expr:str, sparse_tokens=List[str]) -> List[str]:
        try:
            # result = await self.client.query(
            #     collection_name=collection_name,
            ...
            results = await self.client.search(
                ...
                group_by_field="original_chunk",
                group_size=1
            )
            result = [s['entity'] for r in results for s in r]
    ...
    def _integrate_results(
        ...
        # 출현 빈도 반영(20260213 류성원 고도화)
        # chunk별 점수 및 출현 정보 수집
        chunk_score_map = defaultdict(lambda: {
            "scores": [],           # 각 쿼리에서 점수 리스트
            "best_chunk": None,     # 가장 높은 점수의 chunk 데이터
            "best_score": 0.0,
            "query_indices": [],    # 어떤 쿼리에서 나왔는지
        })

        for i in range(num_queries):
            dense_hits = dense_results[i] if i < len(dense_results) else []
            sparse_hits = (sparse_results[i]
            ...
            for chunk in query_results:
                ...
                entry = chunk_score_map[chunk_id]

                # 점수 기록
                entry["scores"].append(current_score)
                entry["query_indices"].append(i)
        ...
        # 최종 정렬 및 정규화(출현 빈도 반영)
        final_results = []
        
        for chunk_id, entry in chunk_score_map.items():
            chunk = entry["best_chunk"]
            if chunk is None:
                continue

            hit_count = len(entry["scores"])                # 몇 개의 쿼리에서 등장했는지
            hit_ratio = hit_count / num_queries             # 출현 비율 (0.0 ~ 1.0)
            avg_score = sum(entry["scores"]) / hit_count    # 평균 점수
            max_score = entry["best_score"]                 # 최고 점수

            # 최종 점수 = 최고점수 기반 + 출현빈도 보너스
            frequency_bonus_weight = 0.3
            integrated_score = max_score * (1.0 + frequency_bonus_weight * hit_ratio)

            chunk["hybrid_score"] = integrated_score
            chunk["_hit_count"] = hit_count
            chunk["_avg_score"] = avg_score
            chunk["_hit_ratio"] = hit_ratio
            chunk["_max_score"] = max_score
            chunk["_source_queries"] = entry["query_indices"]
    ...
    def _apply_rrf(
        ...
        # chunk_id를 dict key로 사용하여 dense/sparse 결과를 자동 병합 (implicit dedup)
        # 같은 chunk_id가 dense와 sparse 양쪽에 존재하면 hybrid_score만 합산됨
        rrf_scores = defaultdict(lambda: {"hybrid_score": 0.0})
        k = self.hybrid_config["rrf_k"]
        
        # Dense 결과 처리 — Distance 타이브레이커 포함
        if dense_results:
            # Distance min-max 정규화용 (타이브레이커 스케일링)
            ...
            distance_weight = self.hybrid_config.get("distance_weight", 0.4)
            ...
            for rank, result in enumerate(dense_results):
                ...
                # Distance 타이브레이커: 동일 RRF 점수 내에서 더 유사한 문서 우선
                # 0.001 스케일이므로 RRF 순위는 보존하면서 동일 순위 내 정렬만 개선
                distance_tiebreaker = 0.0
                if dist_range > 0 and result.get("distance") is not None:
                    normalized_similarity = 1 - ((result["distance"] - min_dist) / dist_range)
                    distance_tiebreaker = normalized_similarity * 0.001

                final_score = dense_weight * rrf_score + distance_tiebreaker
    ...
    async def rerank_and_score_final(
        self,
        base_query: str,
        chunks: List[Dict],
        ...
        # 2. Semantic 점수 계산
        semantic_scores, semantic_method = await self._calculate_semantic_scores(
            base_query,
            processed_chunks
        )
        ...
        # 4. 그룹 최신성 점수 계산 (가중치 > 0일 때만 실행)
        recency_scores = self.recency_scorer.calculate_scores(processed_chunks) if weights.get("group_recency", 0) > 0 else {}
        ...
        for i, chunk in enumerate(processed_chunks):
            ...
            # Exact Match 점수 — config의 final_weights.exact_match 가중치로 제어 (기본 0.0)
            exact_match_score = self._calculate_exact_match_score(chunk) if weights.get("exact_match", 0) > 0 else 0.0

            # 그룹 최신성 점수 — config의 final_weights.group_recency 가중치로 제어 (기본 0.0)
            recency_score = recency_scores.get(chunk_id, 0.0) if weights.get("group_recency", 0) > 0 else 0.0
    ...
    async def _calculate_semantic_scores(
        ...
        # Cross-Encoder 시도 (비동기)
        if self.reranker_url and self.reranker_model:
            try:
                cross_scores = await self._call_reranker_api(query, chunk_texts)
        ...
        if method == "None":
            try:
                vectors = await asyncio.to_thread(self.encode, [query] + chunk_texts)
    ...
    async def _call_reranker_api(self, query: str, documents: List[str]) -> List[float]:
        """리랭커 API 호출 (비동기)"""
        ...
        try:
            # 모델별 포맷팅
            if "telepix" in self.reranker_model:
                formatted_query, formatted_docs = self._format_pixie_spell(query, documents)
            else:
            ...
            # httpx 비동기 호출 (커넥션 재사용)
            client = await self._get_reranker_client()
    ...
    def _format_pixie_spell(
        ...
        def format_query(q: str, instruction: str = None) -> str:
            prefix = (
                '<|im_start|>system\n'
                'Strictly Judge whether the Document meets the requirements based on the Query and the Instruct provided. '
                'Note that the answer can only be "yes" or "no".<|im_end|>\n'
                '<|im_start|>user\n'
            )
            if instruction is None:
                instruction = "Given a internal‑document(Knowledge Base) search query, retrieve relevant passages that answer the query"
        ...
        task = "Given a internal‑document(Knowledge Base) search query, retrieve relevant passages that answer the query"
